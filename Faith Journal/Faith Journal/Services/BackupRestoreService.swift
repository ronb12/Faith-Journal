...
class BackupRestoreService: ObservableObject {
    ...
    private func loadPreferences() {
        if userDefaults.object(forKey: "automaticBackupsEnabled") == nil {
            automaticBackupsEnabled = true
        } else {
            automaticBackupsEnabled = userDefaults.bool(forKey: "automaticBackupsEnabled")
        }
    ...
    func checkAndPerformAutomaticBackup(modelContext: ModelContext) async {
        ...
        guard let lastBackup = lastBackupDate else {
            ...
            _ = try? await createBackup(modelContext: modelContext)
            return
        }
