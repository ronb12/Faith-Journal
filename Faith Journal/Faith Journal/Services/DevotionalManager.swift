...
struct Devotional: Identifiable, Codable {
    ...
    init(id: UUID = UUID(), title: String, scripture: String, content: String, author: String, date: Date, category: String, isFavorite: Bool = false, isCompleted: Bool = false) {
        self.id = id
        self.title = title
...
class DevotionalManager: ObservableObject {
    static let shared = DevotionalManager()
    ...
    func setModelContext(_ context: ModelContext) {
        ...
        print("✅ [DEVOTIONAL] ModelContext set - using UserDefaults (primary) and SwiftData (backup)")
        
        // Verify we can read from SwiftData by checking for existing completions (for backup/migration)
        verifySwiftDataConnection()
        
        // Reload completion status for all devotionals from UserDefaults (primary)
        // This ensures completion status persists across app launches
        reloadCompletionStatusForAllDevotionals()
        
        // If devotionals haven't been loaded yet, trigger loading now that we have modelContext
        if !hasStartedLoading {
            loadDevotionals()
        }
    }
    
    /// Verify SwiftData connection by checking for existing completions
    private func verifySwiftDataConnection() {
        guard let context = modelContext else { return }
        
        do {
            let descriptor = FetchDescriptor<DevotionalCompletion>()
            let allCompletions = try context.fetch(descriptor)
            print("✅ [DEVOTIONAL] SwiftData connection verified - found \(allCompletions.count) saved completions")
            
            // Log details of saved completions
            for completion in allCompletions.prefix(5) {
                print("   - \(completion.devotionalDate): \(completion.isCompleted ? "read" : "unread")")
            }
        } catch {
            print("❌ [DEVOTIONAL] Error verifying SwiftData connection: \(error.localizedDescription)")
        }
    }
    
    /// Reload completion status for all devotionals from UserDefaults (primary) and SwiftData (backup)
    /// This is called after modelContext is set to ensure persistence
    func reloadCompletionStatusForAllDevotionals() {
        print("🔄 [DEVOTIONAL] Reloading completion status for \(devotionals.count) devotionals from UserDefaults...")
        
        // Count completions in UserDefaults
        let dateFormatter = DateFormatter()
        dateFormatter.dateStyle = .short
        dateFormatter.timeStyle = .none
        var userDefaultsCount = 0
        
        // Check UserDefaults for all devotionals
        for devotional in devotionals {
            let key = userDefaultsKey(for: devotional.date)
            if UserDefaults.standard.bool(forKey: key) {
                userDefaultsCount += 1
            }
        }
        print("📊 [DEVOTIONAL] Found \(userDefaultsCount) completions in UserDefaults")
        
        // Also check SwiftData if available (for logging/migration)
        if let context = modelContext {
            context.processPendingChanges()
            do {
                let allCompletionsDescriptor = FetchDescriptor<DevotionalCompletion>()
                let allCompletions = try context.fetch(allCompletionsDescriptor)
                print("📊 [DEVOTIONAL] Found \(allCompletions.count) completions in SwiftData (backup)")
            } catch {
                print("⚠️ [DEVOTIONAL] Error fetching SwiftData completions: \(error.localizedDescription)")
            }
        }
        
        // Update all existing devotionals with their saved completion status
        var reloadedCount = 0
        var checkedCount = 0
        
        for index in devotionals.indices {
            let devotional = devotionals[index]
            checkedCount += 1
            
            print("   Checking devotional \(checkedCount)/\(devotionals.count): \(dateFormatter.string(from: devotional.date))")
            
            let savedStatus = loadCompletionStatus(for: devotional)
            print("   Current status: \(devotionals[index].isCompleted), Saved status: \(savedStatus)")
            
            if devotionals[index].isCompleted != savedStatus {
                devotionals[index].isCompleted = savedStatus
                reloadedCount += 1
                print("✅ [DEVOTIONAL] Reloaded completion status for \(dateFormatter.string(from: devotional.date)): \(savedStatus)")
            }
        }
        
        print("✅ [DEVOTIONAL] Reloaded \(reloadedCount) completion statuses (checked \(checkedCount) devotionals)")
    }
    ...
    private func preloadTodaysDevotional() {
        ...
        // Load saved completion status from UserDefaults (primary) or SwiftData (backup)
        todaysDevotional.isCompleted = loadCompletionStatus(for: todaysDevotional)
    ...
    func loadDevotionals() {
        guard devotionals.count <= 1 && !hasStartedLoading else { return } // Only load once
        hasStartedLoading = true
        isLoading = true
        
        // Ensure modelContext is set before generating
        guard modelContext != nil else {
            print("⚠️ [DEVOTIONAL] Cannot load devotionals - modelContext not set yet")
            isLoading = false
            hasStartedLoading = false // Allow retry when modelContext is set
            return
        }
        
        // Generate devotionals on main thread since we need ModelContext access
        Task { @MainActor [weak self] in
            guard let self = self, let context = self.modelContext else { return }
            
            // Force process pending changes to ensure we're reading from disk
            context.processPendingChanges()
            
            print("🔄 [DEVOTIONAL] Generating devotionals and loading completion status from SwiftData...")
            let generated = self.generate365Devotionals()
            
            // Count how many have completion status
            let completedCount = generated.filter { $0.isCompleted }.count
            print("✅ [DEVOTIONAL] Loaded \(generated.count) devotionals, \(completedCount) marked as read")
            
            self.devotionals = generated
    ...
    /// Get UserDefaults key for a devotional date
    private func userDefaultsKey(for date: Date) -> String {
        let dateFormatter = ISO8601DateFormatter()
        dateFormatter.formatOptions = [.withFullDate, .withDashSeparatorInDate]
        return "devotional_completed_\(dateFormatter.string(from: date))"
    }
    
    private func loadCompletionStatus(for devotional: Devotional) -> Bool {
        // PRIMARY: Use UserDefaults for reliable persistence
        let key = userDefaultsKey(for: devotional.date)
        let userDefaultsStatus = UserDefaults.standard.bool(forKey: key)
        
        if userDefaultsStatus {
            let dateFormatter = DateFormatter()
            dateFormatter.dateStyle = .short
            dateFormatter.timeStyle = .none
            print("✅ [DEVOTIONAL] Loaded completion status from UserDefaults for \(dateFormatter.string(from: devotional.date)): true")
        }
        
        // SECONDARY: Also check SwiftData if available (for migration/backup)
        if let context = modelContext {
            let calendar = Calendar.current
            let startOfDay = calendar.startOfDay(for: devotional.date)
            guard let endOfDay = calendar.date(byAdding: .day, value: 1, to: startOfDay) else {
                return userDefaultsStatus
            }
            ...
            do {
                ...
                if let completion = results.first, completion.isCompleted {
                    // If SwiftData has it but UserDefaults doesn't, sync to UserDefaults
                    if !userDefaultsStatus {
                        UserDefaults.standard.set(true, forKey: key)
                        print("🔄 [DEVOTIONAL] Migrated completion status from SwiftData to UserDefaults")
                    }
                    return true
                }
            } catch {
                // If SwiftData fetch fails, just use UserDefaults
                print("⚠️ [DEVOTIONAL] SwiftData fetch failed, using UserDefaults: \(error.localizedDescription)")
            }
        }
    ...
    private func saveCompletionStatus(for devotional: Devotional, isCompleted: Bool) {
        ...
        dateFormatter.dateStyle = .short
        dateFormatter.timeStyle = .none
        let dateString = dateFormatter.string(from: devotional.date)
        
        // PRIMARY: Save to UserDefaults immediately (most reliable)
        let key = userDefaultsKey(for: devotional.date)
        UserDefaults.standard.set(isCompleted, forKey: key)
        UserDefaults.standard.synchronize() // Force immediate write to disk
        print("💾 [DEVOTIONAL] Saved completion status to UserDefaults for \(dateString): \(isCompleted)")
        print("   Key: \(key)")
        
        // SECONDARY: Also save to SwiftData if available (for backup/migration)
        if let context = modelContext {
            let calendar = Calendar.current
            let startOfDay = calendar.startOfDay(for: devotional.date)
            guard let endOfDay = calendar.date(byAdding: .day, value: 1, to: startOfDay) else {
                print("⚠️ [DEVOTIONAL] Invalid date, but UserDefaults save succeeded")
                // Still sync to Firebase
                syncCompletionToFirebase(for: devotional, isCompleted: isCompleted)
                return
            }
            ...
            do {
                ...
                if let existing = results.first {
                    ...
                    existing.devotionalId = devotional.id
                    print("✅ [DEVOTIONAL] Updated SwiftData completion status for \(dateString): \(isCompleted)")
                } else {
                    ...
                    print("✅ [DEVOTIONAL] Created SwiftData completion status for \(dateString): \(isCompleted)")
                }
                
                // Save to SwiftData (backup)
                try context.save()
                context.processPendingChanges()
                print("💾 [DEVOTIONAL] Also saved to SwiftData for backup")
                
                // Sync to Firebase
                syncCompletionToFirebase(for: devotional, isCompleted: isCompleted)
                
            } catch {
                print("⚠️ [DEVOTIONAL] SwiftData save failed, but UserDefaults save succeeded: \(error.localizedDescription)")
                // UserDefaults save succeeded, so we're good. Still sync to Firebase.
                syncCompletionToFirebase(for: devotional, isCompleted: isCompleted)
            }
        } else {
            print("ℹ️ [DEVOTIONAL] ModelContext not available, using UserDefaults only")
            // Sync to Firebase even without SwiftData
            syncCompletionToFirebase(for: devotional, isCompleted: isCompleted)
        }
    }
    
    /// Sync completion status to Firebase
    private func syncCompletionToFirebase(for devotional: Devotional, isCompleted: Bool) {
        // Create a DevotionalCompletion object for Firebase sync
        let completion = DevotionalCompletion(
            devotionalId: devotional.id,
            devotionalDate: devotional.date,
            isCompleted: isCompleted
        ...
        Task { @MainActor in
            print("🔄 [DEVOTIONAL] Syncing completion to Firebase...")
            await FirebaseSyncService.shared.syncDevotionalCompletion(completion)
            print("✅ [DEVOTIONAL] Firebase sync completed for \(devotional.date)")
        }
    }
    
    func generate365Devotionals() -> [Devotional] {
        // Note: Function name kept as generate365Devotionals for compatibility,
        // but it now generates 1000 devotionals
        let calendar = Calendar.current
        let today = Date()
        var allDevotionals: [Devotional] = []
        allDevotionals.reserveCapacity(1000) // Pre-allocate for 1000 devotionals
        ...
        for dayOffset in 0..<1000 {
            ...
            // Load saved completion status from UserDefaults (primary) or SwiftData (backup)
            devotional.isCompleted = loadCompletionStatus(for: devotional)
    ...
    func markAsCompleted(_ devotional: Devotional) {
        if let index = devotionals.firstIndex(where: { $0.id == devotional.id }) {
            ...
            dateFormatter.dateStyle = .short
            dateFormatter.timeStyle = .none
            print("🔄 [DEVOTIONAL] Marking devotional as \(newStatus ? "read" : "unread"): \(dateFormatter.string(from: devotionals[index].date))")
            
            // Save to persistent storage
            saveCompletionStatus(for: devotionals[index], isCompleted: newStatus)
            
            // Verify immediately (UserDefaults is instant)
            let key = self.userDefaultsKey(for: self.devotionals[index].date)
            let verificationStatus = UserDefaults.standard.bool(forKey: key)
            
            if verificationStatus == newStatus {
                print("✅ [DEVOTIONAL] Verified: Completion status persisted to UserDefaults for \(dateFormatter.string(from: self.devotionals[index].date))")
                
                // Ensure the array is updated
                if let currentIndex = self.devotionals.firstIndex(where: { $0.id == self.devotionals[index].id }) {
                    if self.devotionals[currentIndex].isCompleted != verificationStatus {
                        self.devotionals[currentIndex].isCompleted = verificationStatus
                        print("✅ [DEVOTIONAL] Updated array with persisted status")
                    }
                }
                ...
                print("⚠️ [DEVOTIONAL] Warning: Completion status verification failed for \(dateFormatter.string(from: self.devotionals[index].date))")
                print("   Expected: \(newStatus), Got: \(verificationStatus)")
                print("   Key: \(key)")
            ...
            print("⚠️ [DEVOTIONAL] Cannot mark as completed - devotional not found in array")
        }
    }
    ...
    func toggleFavorite(_ devotional: Devotional) {
        if let index = devotionals.firstIndex(where: { $0.id == devotional.id }) {
            ...
            print("✅ [DEVOTIONAL] Toggled favorite status for \(devotional.title): \(devotionals[index].isFavorite)")
        }
    }
}
