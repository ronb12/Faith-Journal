    ...
    static func from(_ error: Error) -> AppError {
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet, .networkConnectionLost:
                return .networkError("No internet connection. Your changes will be saved locally and synced when online.")
            case .timedOut:
        ...
        // Convert to NSError to access domain and code
        let nsError = error as NSError
        
        // Handle Firebase errors
        #if canImport(FirebaseFirestore)
        if nsError.domain.contains("FIR") {
            if nsError.code == 7 { // Permission denied
                return .permissionDenied
            }
            if nsError.code == 14 { // Unavailable
                return .networkError("Service temporarily unavailable. Your data is saved locally.")
            }
        }
        #endif
