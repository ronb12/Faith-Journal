...
struct BroadcastStreamView_HLS: View {
    ...
    private var sessionParticipants: [LiveSessionParticipant] {
        ...
        return allParticipants.filter { $0.sessionId == session.id }
    }
    ...
    // Chat messages for this session (filtered by current room)
    var sessionMessages: [ChatMessage] {
        ...
        return unique.filter { $0.roomId == nil }
    }
                ...
                if isHost {
                    ...
                    if let session = session, session.isActive, elapsed >= 5 {
                        ...
                        do {
                            ...
                            print("✅ [SESSION] Session ended automatically when host left: \(session.title)")
    ...
    private func stopAndDismiss() {
        ...
        // If host is stopping, end the session
        if let session = session, isHost && session.isActive {
            session.isActive = false
            session.endTime = Date()
...
struct LiveStreamChatOverlay: View {
    ...
    let onSend: () -> Void
    let onDismiss: () -> Void
    var roomName: String? = nil // Optional room name for breakout room chat
    ...
    var body: some View {
        VStack(spacing: 0) {
            // Room indicator if in breakout room
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 8) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                // Message bubble
                VStack(alignment: .leading, spacing: 4) {
                    Text(showingTranslation && translatedText != nil ? translatedText! : message.message)
                        .font(.subheadline)
                        .foregroundColor(.white)
                    
                    // Translation toggle
                    if !message.message.isEmpty && message.originalLanguage != translationService.defaultTargetLanguage {
                        Button(action: {
                            ...
                            HStack(spacing: 4) {
                                if isTranslating {
                                    ...
                                    Image(systemName: showingTranslation ? "text.bubble.fill" : "text.bubble")
                                }
                                Text(showingTranslation ? "Show Original" : "Translate")
                            }
                            .font(.caption2)
                            .foregroundColor(.white.opacity(0.7))
                        }
    ...
    private func translateMessage() {
        ...
        Task {
            let translated = await translationService.translate(
                ...
                to: translationService.defaultTargetLanguage
            )
