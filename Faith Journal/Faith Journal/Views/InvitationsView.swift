...
struct JoinByCodeView: View {
    ...
    private func joinByCode() {
        ...
        if let localInvitation = invitations.first(where: { $0.inviteCode.uppercased() == code && $0.isValid }) {
        ...
        Task {
            await MainActor.run {
                errorMessage = "Looking up invitation..."
            }
            
            // Fetch invitation by code from Firebase
            if let fetchedInvitation = await FirebaseSyncService.shared.fetchInvitationByCode(code) {
                await MainActor.run {
                    // Check if invitation already exists locally to prevent duplicates
                    let invitationId = fetchedInvitation.id
                    let existingInvitationQuery = FetchDescriptor<SessionInvitation>(
                        predicate: #Predicate<SessionInvitation> { invitation in
                            invitation.id == invitationId
                        }
                    )
                    
                    if let existingInvitation = try? modelContext.fetch(existingInvitationQuery).first {
                        // Invitation already exists - use the existing one
                        print("✅ [JOIN] Invitation already exists locally, using existing: \(code)")
                        joinWithInvitation(existingInvitation)
                    } else {
                        // Invitation doesn't exist - insert it
                        modelContext.insert(fetchedInvitation)
                        do {
                            try modelContext.save()
                            print("✅ [JOIN] Found invitation via Firebase and saved locally: \(code)")
                            joinWithInvitation(fetchedInvitation)
                        } catch {
                            errorMessage = "Error saving invitation: \(error.localizedDescription)"
                            showingError = true
                        }
                    }
                }
                ...
                await MainActor.run {
                    errorMessage = "Invitation code not found or has expired. Please check the code and try again."
                    showingError = true
                }
            }
        }
    ...
    private func joinWithInvitation(_ invitation: SessionInvitation) {
        ...
        if let localSession = try? modelContext.fetch(sessionQuery).first {
            joinWithSession(invitation: invitation, session: localSession)
            return
        }
        
        // If session not found locally, fetch from Firebase
        Task {
            await MainActor.run {
                errorMessage = "Looking up session..."
            }
            
            if let fetchedSession = await FirebaseSyncService.shared.fetchSessionById(sessionId) {
                await MainActor.run {
                    // Check if session is still active
                    guard fetchedSession.isActive else {
                        errorMessage = "This session is no longer active"
                    ...
                    // Check if session already exists locally to prevent duplicates
                    let existingSessionQuery = FetchDescriptor<LiveSession>(
                        predicate: #Predicate<LiveSession> { session in
                            session.id == sessionId
                        }
                    )
                    
                    if let existingSession = try? modelContext.fetch(existingSessionQuery).first {
                        // Session already exists - use the existing one and update if needed
                        print("✅ [JOIN] Session already exists locally, using existing session: \(sessionId)")
                        joinWithSession(invitation: invitation, session: existingSession)
                    } else {
                        // Session doesn't exist - insert it
                        modelContext.insert(fetchedSession)
                        do {
                            try modelContext.save()
                            print("✅ [JOIN] Found session via Firebase and saved locally: \(sessionId)")
                            joinWithSession(invitation: invitation, session: fetchedSession)
                        } catch {
                ...
                await MainActor.run {
                    errorMessage = "Session not found or is no longer available. Please check the invitation code."
                    invitation.status = .expired
                    do {
                        try modelContext.save()
                        Task {
    ...
    private func joinWithSession(invitation: SessionInvitation, session: LiveSession) {
        ...
        do {
            ...
            // Sync invitation status, participant, and session update to Firebase
            Task {
                await FirebaseSyncService.shared.syncSessionInvitation(invitation)
                ...
                print("✅ [JOIN] Synced invitation, participant, and count to Firebase")
                print("✅ [JOIN] Host will be notified via Firebase listener when participant is detected")
            }
