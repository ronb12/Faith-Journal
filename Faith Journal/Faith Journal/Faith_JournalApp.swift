...
struct Faith_JournalApp: App {
    ...
    init() {
        ...
        // CRITICAL: Initialize Firebase BEFORE any views are created
        // This ensures FirebaseApp.configure() is called before Auth is accessed
        print("🔍 [FIREBASE] Initializing Firebase at app startup...")
    ...
    var body: some Scene {
        WindowGroup {
            ZStack {
                // Ensure window background matches app - extends under all safe areas
            ...
            .onOpenURL { url in
                appNavigation.handleDeepLink(url)
            }
                .task {
                ...
                await MainActor.run {
                    ...
                    // Firebase is already initialized in init() - just verify it's configured
                    if FirebaseInitializer.shared.isConfigured {
                        print("✅ [FIREBASE] Firebase already configured at app startup")
                    } else {
                        print("⚠️ [FIREBASE] Firebase not configured, attempting initialization...")
                        FirebaseInitializer.shared.initialize()
                    }
...
struct FullScreenWindowConfigurator: UIViewControllerRepresentable {
    ...
    func updateUIViewController(_ uiViewController: UIViewController, context: Context) {
        DispatchQueue.main.async {
            // Configure window background to match app
            if let windowScene = UIApplication.shared.connectedScenes.first as? UIWindowScene {
                for window in windowScene.windows {
                    // Set window background to match app
                    window.backgroundColor = UIColor.systemGroupedBackground

                    // Recursively configure all view controllers in the hierarchy
                    func configureViewController(_ vc: UIViewController) {
                        ...
                        // Configure all child view controllers
                        for child in vc.children {
                            configureViewController(child)
                        }

                        // If it's a navigation controller, configure it
                        if let navController = vc as? UINavigationController {
                            navController.edgesForExtendedLayout = .all
                            navController.extendedLayoutIncludesOpaqueBars = true
                            navController.view.backgroundColor = UIColor.systemGroupedBackground
                            navController.view.clipsToBounds = false
                        }

                        // If it's a tab bar controller, configure it
                        if let tabController = vc as? UITabBarController {
                            tabController.edgesForExtendedLayout = .all
                            tabController.extendedLayoutIncludesOpaqueBars = true
                            tabController.view.backgroundColor = UIColor.systemGroupedBackground
                            tabController.view.clipsToBounds = false
                        }
                    }

                    // Configure root view controller and all children
                    if let rootViewController = window.rootViewController {
                        configureViewController(rootViewController)
                    }
                }
            }
        }
