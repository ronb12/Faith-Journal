...
final class LiveSessionParticipant {
    ...
    var isBroadcaster: Bool = false // Whether this participant is a broadcaster (can stream) or audience (viewer only)
    
    init(sessionId: UUID, userId: String, userName: String, isHost: Bool = false, isBroadcaster: Bool = false) {
        self.id = UUID()
