...
class ExportHelper {
    ...
    func exportJournalEntryAsPDF(_ entry: Any) throws -> URL {
        guard #available(iOS 17.0, *), let entry = entry as? JournalEntry else {
            throw NSError(domain: "ExportHelper", code: 1, userInfo: [NSLocalizedDescriptionKey: "PDF export requires iOS 17+"])
        }
        let pdfMetaData = [
    ...
    func exportJournalEntryAsText(_ entry: Any) -> String {
        guard #available(iOS 17.0, *), let entry = entry as? JournalEntry else {
            return "Export as text requires iOS 17+"
        }
        var text = """
        \(entry.title)
...
struct JournalExportView: View {
    ...
    var body: some View {
        if #available(iOS 17.0, *), let entry = entry as? JournalEntry {
            NavigationView {
                Form {
            ...
            Text("Export is only available on iOS 17+")
        }
    }
}
