                        ...
                        VStack(spacing: 24) {
                            ...
                            Button(action: {
                                ...
                                Task {
                                    ...
                                    #if canImport(FirebaseAuth)
                                    if FirebaseInitializer.shared.isConfigured, Auth.auth().currentUser == nil {
                                        do {
                                            _ = try await Auth.auth().signInAnonymously()
                                            print("✅ [DEMO] Firebase anonymous session created (server admin tools still need an email or Apple sign-in for your account).")
                                        } catch {
                                            print("⚠️ [DEMO] Anonymous sign-in failed: \(error.localizedDescription)")
                                        }
                                    }
                                    #endif
    ...
    private func signInToFirebaseWithApple(appleIDCredential: ASAuthorizationAppleIDCredential) async {
        ...
        do {
            ...
            if let code = AuthErrorCode.Code(rawValue: nsError.code) {
                print("❌ [FIREBASE AUTH] AuthErrorCode: \(code)")
                switch code {
                case .networkError:
