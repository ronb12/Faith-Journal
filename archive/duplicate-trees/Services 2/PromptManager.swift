...
class PromptManager: ObservableObject {
    ...
    private var promptLibrary: [JournalPrompt] = []
    private let calendar = Calendar.current
    private var isInitialized = false
    
    private init() {
        print("🚀 [LAUNCH] PromptManager.init started")
        // Initialize lazily to avoid crashes during app startup
        // The library will be created on first access
        print("🚀 [LAUNCH] PromptManager.init completed (lazy initialization)")
    }
    ...
    private func ensureInitialized() {
        ...
        // Wrap in a safe block to catch any unexpected issues
        let library = PromptManager.createPromptLibrary()
        if library.isEmpty {
            // If library creation fails, use fallback prompts
            print("⚠️ PromptManager: Library creation returned empty, using fallback")
            promptLibrary = [
                JournalPrompt(promptText: "What's on your heart today?", category: .general),
                JournalPrompt(promptText: "How has God blessed you today?", category: .gratitude),
                JournalPrompt(promptText: "What do you want to pray about?", category: .prayer)
            ]
        } else {
            promptLibrary = library
        }
        
        // Load daily prompt and favorites safely
        loadDailyPrompt()
        loadFavorites()
    }
    ...
    func getPromptForDay(_ day: Int) -> JournalPrompt {
        ensureInitialized()
        guard !promptLibrary.isEmpty else {
            return JournalPrompt(promptText: "What's on your heart today?", category: .general)
        }
    ...
    func loadDailyPrompt() {
        ensureInitialized()
        // The actual loading is now done in ensureInitialized to avoid recursion
    }
