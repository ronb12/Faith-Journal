...
final class SessionInvitation {
    ...
    init(
        ...
        self.status = .pending
        self.createdAt = Date()
        // Default expiration: 30 days from now (more reasonable for shared codes)
        // If expiresAt is nil, set to 30 days from creation
        self.expiresAt = expiresAt ?? Calendar.current.date(byAdding: .day, value: 30, to: Date())
    }
